...
namespace FishInvader
{
    public partial class Jellyfish
    {
        ...
        {
            Jellyfishfilepath = $"images/originalfish/jellyf1sh20.png";
            Jellyfishfilepath2 = $"images/originalfish/jellyf2sh20.png";
            ...
            for (_size = 1; _size < 5; _size++)
            {
                ...
                {
                    Jellyfishfilepath = $"images/originalfish/jellyf1sh{_size}.png";
                    Jellyfishfilepath2 = $"images/originalfish/jellyf2sh{_size}.png";
                    _helth = _size;
                    break;
                }
            }
            ...
            if (!AirSpace.TalkingToPng)
            {
                ...
                foreach (Projectile projectile in projectiles)
                {
                    if (!IsPnj && !IsShop)
                    {
                        ...
                        {
                            if ((projectile.X - projectile.Width) <= (_x + _width) && (projectile.Y - projectile.Height) <= (_y + _height) && (projectile.X + projectile.Width) >= (_x - _width) && (projectile.Y + projectile.Height) >= (_y - _height))
                            {
                        ...
                        {
                            if ((projectile.X - projectile.Slimewidth) <= (_x + _width) && (projectile.Y - projectile.Slimeheight) <= (_y + _height) && (projectile.X + projectile.Slimewidth) >= (_x - _width) && (projectile.Y + projectile.Slimeheight) >= (_y - _height))
                            {
                ...
                if (_y <= -100)
                {
                    ...
                    else
                    {
                        ...
                        for (_size = 5; _size < 16; _size++)
                        {
                            ...
                            {
                                originaljellyfish = Image.FromFile($"images/originalfish/jellyf1sh" + _size + ".png");
                                originaljellyfish2 = Image.FromFile($"images/originalfish/jellyf2sh" + _size + ".png");
                                _helth = _size;
                                break;
                            }
